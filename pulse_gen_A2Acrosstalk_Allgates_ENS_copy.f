...
! omega_p; angular freq as default
         pi = 4.d0*datan(1.d0)
         freq_range_1 = freq(1,1)
         freq_range_2 = freq(num_ion,1)
         freq_interval = (freq_range_2- freq_range_1)/3
...
!          freq_range(2,igate)=freq_range_1+igate*freq_interval; ! If there is a sweep over i we should add i*sweep_inc
          ...
          freq_range(1,6)=freq_range_1; ! If there is a sweep over i we should add i*sweep_inc
          freq_range(2,6)=freq_range_2; ! If there is a sweep over i we should add i*sweep_inc

          freq_range(1,4)=freq_range_1; ! If there is a sweep over i we should add i*sweep_inc
          freq_range(2,4)=freq_range_2; ! If there is a sweep over i we should add i*sweep_inc

         

          freq_range(1,5)=freq_range_1; ! If there is a sweep over i we should add i*sweep_inc
          freq_range(2,5)=freq_range_2; ! If there is a sweep over i we should add i*sweep_inc
...
!            nrow_len=num_ion*(nstab_order+1)
     ...
     .       (tau,freq,vec_null2,l_min,l_max,nrow_len,num_null_dim)
            ...
            l_in=1+nint(dble(igate-4)*dble(num_null_dim)/3.d0)
            l_fi=nint(dble(igate-4+1)*dble(num_null_dim)/3.d0)
...
!               write(6,*) realtime_pulse(pulse_str,l_min,l_max,
!     .          igate,i,200.d0,tau)
               
!               write(6,*) Sqrt(Sum(abs(pulse(:)*pulse(:))))
                  
!                  write(20+igate, '(I4,A,I4,A,f0.16)')
!     .            i,',',pulse_maxind(i),',',pulse1_maxamp(i) 
...
!     enddo
     ...
     .           dmat_str,l_minn,l_maxx,l_minn,l_maxx)
            do i = 0, imax
               ...
               do ngst=56,56  ! In order to sweep over all the gate configuratioins we need to start ngst from zero
                  write(6,*) ibits(ngst,4,1),ibits(ngst,5,1),
     .             ibits(ngst,6,1)
                  do j=1,num_gate
...
!               print *, ent_ab_ideal
               ...
               do j = 1, num_gate
                   write(m+50, '(I1, A)', advance='no') gate_stat(j)
               enddo
...
!      function realtime_pulse(pulse_str,l_min_pulse,l_max_pulse,
!     .     igate,i,time,tau,l_pulse_dim,imax) result(m)
!      parameter (num_ion=7,num_qubit=5,num_gate=2)
!      integer, intent(in) :: igate,i,l_pulse_dim,imax
!      integer, intent(in) :: l_max_pulse,l_min_pulse
!      double precision :: time, tau
!      dimension pulse_str(num_gate,l_pulse_dim,0:imax)
!      double precision :: m
!      m=0
!      do ifreq=1, l_max_pulse-l_min_pulse+1
!         m=m+pulse_str(igate,ifreq,i)*dsin(time*ifreq/tau)
!      enddo
!      end function realtime_pulse
...
!     dimension vec_null(l_max-l_min+1,l_max-l_min+1-nrow_len)
      dimension vec_null(l_max-l_min+1,l_in:l_fi)
...
*
     ...
     $             WORK, LWORK, INFO )
      ...
      do i=l_in, l_fi
         ii=i-l_in+1
      pulse(:) = pulse(:) + VT(isolution,ii)*vec_null(:,i)
      enddo
