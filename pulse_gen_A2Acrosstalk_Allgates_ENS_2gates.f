...
!     print *, sweep_inc
      ...
      gate_on=dble(2)
      do igate=1,10
         if (igate.NE.1.AND.igate.NE.8) cycle
...
!            nrow_len=num_ion*(nstab_order+1)
     ...
     .       (tau,freq,vec_null2,l_min,l_max,nrow_len,num_null_dim)
            ...
            l_in=1+nint(dble(r)*dble(num_null_dim)/gate_on)
            l_fi=nint(dble(r+1)*dble(num_null_dim)/gate_on)
                   write(6,*) l_in,l_fi
...
!     enddo
     ...
     .           dmat_str,l_minn,l_maxx,l_minn,l_maxx)
            do i = 0, imax
               ...
               do ngst=129,129 ! In order to sweep over all the gate configuratioins we need to start ngst from zero
                  write(6,*) ibits(ngst,0,1),ibits(ngst,7,1)
