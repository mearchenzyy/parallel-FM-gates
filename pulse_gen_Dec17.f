...
!      parameter (num_activegate=2)
      ...
      integer, allocatable :: gates_on(:)
c      integer gate_stat(num_gate)
c      dimension l_range(2,num_gate)
c      dimension vec_null(num_basis,num_basis-num_ion)
c     dimension pulse(num_basis)
      double precision, allocatable :: vec_null2(:,:)
      double precision, allocatable :: vec_null_ordered(:,:)
      double precision, allocatable :: vec_null_conc(:)
      double precision, allocatable :: l_basis_freq(:)
      double precision, allocatable :: ll_ordered(:)
      double precision, allocatable :: vec_null(:,:), pulse(:),dmat(:,:)
      double precision, allocatable :: pulse_a(:), pulse_b(:)
      double precision, allocatable :: pulse_str(:,:,:)
      double precision, allocatable :: dmat_str(:,:)
      double precision, allocatable :: pulset(:,:,:)
      double precision, allocatable :: pulset_max(:,:,:)
      integer, allocatable :: l_min_str(:,:)
      integer, allocatable :: l_max_str(:,:)
      double precision, allocatable :: pulse_maxamp(:,:),
     .pulse_power(:,:)
      integer, allocatable ::  pulse_maxind(:,:)
      double precision number 
      integer l_max1
      integer l_max2
      integer l_min1
      integer l_min2
      integer gate_stat(num_gate)
      integer, allocatable :: nsub_set(:,:)
     ...
     . filename_maxx
      ...
      nstab_order = 0
      ngate_on=7
      tautau = 1531.39458393d0
      Allocate(gates_on(ngate_on))
      gates_on=[1,2,3,4,5,6,7]
c=====================
c Parameter definition
c---------------------
c num_ion := # ions in the ion chain

c num_qubit := # ions that are used as the qubits

c param_ld(i,j,k) := eta_{ij} (Lamb-Dicke parameters); 
c k=1,2,3 -> x,y,z modes. j:= qubit index, i:= mode index

c freq(i,k) := i'th mode frequencies in direction k (MHz)

c num_gate := # 2Q-gates to be parallelized using mode k

c index_gate(i,j) := ion indices (i=1,2 for two ions) for the j'th 2Q-gate

c freq_range(i,j) := min(i=1)-max(i=2) frequencies to use for the j'th 2Q-gate pulse

c tau := gate duration (microsecond)

c num_basis := # sine-basis functions used for each 2Q-gate
c-----------------------------------------------------------
!     Gate duration 501 microseconds as an example
      nrow_len=(nstab_order+1)*num_ion
      print *, int(nrow_len/num_ion)-1
      print *, nrow_len
!     print *, nrow_len
      imax=0
      len_tau=1
      ...
      do k=1,ngate_on
         igate=gates_on(k)
         filename_maxx=''
         write(filename_maxx,'(A,i0,A)')
     .    'time_domain_max/time_domain_max_gate',igate,'.txt'
         open(300+igate,FILE=trim(filename_maxx))
      enddo
      do ntau=1,len_tau
...
! qubit 1, mode x
         ...
         param_ld(2,1,1) = -0.025d0
         param_ld(3,1,1) = -0.040d0
         param_ld(4,1,1) = +0.045d0
         param_ld(5,1,1) = -0.033d0
         param_ld(6,1,1) = +0.001d0
         param_ld(7,1,1) = +0.041d0
! qubit 2, mode x
         param_ld(1,2,1) = +0.030d0
         param_ld(2,2,1) = +0.048d0
         param_ld(3,2,1) = +0.031d0
         param_ld(4,2,1) = -0.002d0
         param_ld(5,2,1) = -0.028d0
         param_ld(6,2,1) = +0.030d0
         param_ld(7,2,1) = +0.028d0
! qubit 3, mode x
         param_ld(1,3,1) = -0.052d0
         param_ld(2,3,1) = -0.023d0
         param_ld(3,3,1) = +0.030d0
         param_ld(4,3,1) = -0.032d0
         param_ld(5,3,1) = +0.002d0
         param_ld(6,3,1) = +0.031d0
         param_ld(7,3,1) = +0.022d0
! qubit 4, mode x
         param_ld(1,4,1) = +0.052d0
         param_ld(2,4,1) = -0.038d0
         param_ld(3,4,1) = -0.020d0
         param_ld(4,4,1) = -0.035d0
         param_ld(5,4,1) = +0.018d0
         param_ld(6,4,1) = +0.034d0
         param_ld(7,4,1) = +0.019d0
! qubit 5, mode x
         param_ld(1,5,1) = -0.023d0
         param_ld(2,5,1) = +0.047d0
         param_ld(3,5,1) = -0.045d0
         param_ld(4,5,1) = +0.003d0
         param_ld(5,5,1) = +0.031d0
         param_ld(6,5,1) = +0.027d0
         param_ld(7,5,1) = +0.000d0
! freqeuencies, mode x
!         freq(1,1) = 2.951d0
!         freq(2,1) = 2.973d0
!         freq(3,1) = 2.993d0
...
!         freq(6,1) = 3.038d0
!         freq(7,1) = 3.054d0
! omega_p; angular freq as default
         pi = 4.d0*datan(1.d0)
         smode_freq_range =  freq(num_ion,1)- freq(1,1)
  
         freq_range_1 = freq(1,1)-0.5d0*smode_freq_range 
         freq_range_2 = freq(num_ion,1)+0.5d0*smode_freq_range 
...
!     ion2_ent=2
      ...
      do igate=1,num_gate
          write(filename_max,'(A,i0,A)')
     .     'pulse_maxamp/max_ampind_gate',igate,'.txt'
          open(20+igate,file=trim(filename_max))
      enddo
      
c      freq_range_old=0.d0



     
      l_pulse_dim = (freq_range_2-freq_range_1)*tau+1
      write(6,*) freq_range_2,freq_range_1
!      Allocate(pulse1_str(l_pulse_dim,0:imax))
     
      Allocate(pulse_maxamp(num_gate,0:imax))
      Allocate(pulse_maxind(num_gate,0:imax))
      
      Allocate(pulse_power(num_gate,0:imax))
      pulse_maxamp=0.d0
c      Allocate(ndim_pulse1_str(0:imax))
      Allocate(l_min_str(num_gate,0:imax))
      Allocate(l_max_str(num_gate,0:imax))
!      Allocate(pulse2_str(l_pulse_dim,0:imax))
      

      Allocate(pulse_str(num_gate,l_pulse_dim,0:imax))
     
     
      n_disc=100000
      Allocate(pulset(num_gate,n_disc,0:imax))
      pulset=0.d0
...
!      write(6,*) gates_on
      ...
      nonbin_gate_stat=0
      do j=1, ngate_on
         nonbin_gate_stat=nonbin_gate_stat+2**(gates_on(j)-1)
      enddo 
...
!         write(6,*) igate,'gate Iteration'
        
       do i=0,imax
!          freq_range(1,igate)=freq_range_1+(igate-1)*freq_interval; ! If there is a sweep over i we should add i*sweep_inc
!          freq_range(2,igate)=freq_range_1+igate*freq_interval; ! If there is a sweep over i we should add i*sweep_inc

          freq_range(1,igate)=freq_range_1; ! If there is a sweep over i we should add i*sweep_inc
          freq_range(2,igate)=freq_range_2; ! If there is a sweep over i we should add i*sweep_inc

     
         

!          write(6,*) freq_range(1,4),freq_range(2,4)
!          write(6,*) freq_range(1,5),freq_range(2,5)
!          write(6,*) freq_range(1,6),freq_range(2,6)

c          stop
          
c         do igate=1,num_gate
...
c            if (freq_range(1,igate).eq.freq_range_old(1,igate) .AND.
c     .           freq_range(2,igate).eq.freq_range_old(2,igate)) cycle
...
!            nrow_len=num_ion*(nstab_order+1)
            Allocate(vec_null2(l_max-l_min+1,l_max-l_min+1))
            call null_space
     .       (tau,freq,vec_null2,l_min,l_max,nrow_len,num_null_dim)
         
            
       
            Allocate(vec_null(l_max-l_min+1,num_null_dim))
            Allocate(vec_null_ordered(l_max-l_min+1,num_null_dim))
            do ivec=1,num_null_dim
               vec_null(:,ivec) = vec_null2(:,l_max-l_min+1-ivec+1)
            enddo
            deAllocate(vec_null2)
            nrow_len = l_max-l_min+1-num_null_dim
            Allocate(pulse(l_max-l_min+1))
            ion1 = index_gate(1,igate); ion2 = index_gate(2,igate)
            ...
            do ivec=1, num_null_dim
                ...
                filename=''
                write(filename,'(A,i0,A,i0,A)') 'nullvec/nullvecgate'
     .           ,igate,'ivec',ivec,'.txt'
                open(44+igate*(num_null_dim),file=filename)
                do ll=1, l_max-l_min+1
                   write(44+igate*(num_null_dim),*)
     .              (ll+l_min-1), vec_null(ll,ivec)
                enddo
            ...
            Allocate(vec_null_conc(num_null_dim))
            Allocate(l_basis_freq(l_max-l_min+1))
            Allocate(ll_ordered(num_null_dim))
           
            do ll=1, num_null_dim
               ll_ordered(ll)=99999
            enddo
 


            do ii=1,l_max-l_min+1
               l_basis_freq(ii)=ii+l_min-1
            enddo
            do ll=1, num_null_dim
               
               vec_null_conc(ll)=SUM((abs(vec_null(:,ll))**2
     .         /SUM(abs(vec_null(:,ll))**2))*l_basis_freq)
              
              
            enddo

!            write(6,*) ANY(MASK=(l_basis_freq==l_min+6)),'this element '
            ...
            do ll=1, num_null_dim
               ...
               s_old=99999999
               ind=1
               do lll=1, num_null_dim
                  if(ANY(MASK=(ll_ordered(1:ll)==lll))) goto 24
                  
                  
                  s_new=min(s_old,vec_null_conc(lll))
                  if(s_new.NE.s_old) then
                     ind=lll
                  endif
                  s_old=s_new
 24               continue
...
!            write(6,*) vec_null_conc
            do ll=1, num_null_dim
               ii=ll_ordered(ll)
               vec_null_ordered(:,ll)=vec_null(:,ii)
            enddo

            l_in=int((freq(k,1)/(2.d0*pi))*tau)-5-l_min+1
!            l_in=1+nint(dble(k-1)*dble(num_null_dim)/dble(ngate_on))
!            l_fi=nint(dble(k)*dble(num_null_dim)/dble(ngate_on))
            l_fi=l_in+10
!            write(6,*) int((freq(k,1)/(2.d0*pi))*tau)-5, 'This is'
            
            write(6,*) l_in,l_fi
!            stop
            if (l_fi.gt.num_null_dim) then
               l_fi=num_null_dim
            endif
     
            Allocate(vec_null2(l_max-l_min+1,l_fi-l_in+1))
            do ivec=l_in,l_fi
               vec_null2(:,ivec-l_in+1) = vec_null_ordered(:,ivec)
            enddo

!            write(6,*) l_in,l_fi,l_max-l_min+1
  
            call gate_pulse(tau,freq,l_min,l_max,param_ld,
     .      vec_null2,
     .           ion1,ion2,pulse,nrow_len,l_in,l_fi)
            deAllocate(vec_null2)
             l_min_str(igate,i) = l_min
             l_max_str(igate,i) = l_max

          
c               l_min1=l_min
c               l_max1=l_max
c               Allocate(pulse1(l_max1-l_min1+1))
c     pulse1=pulse
               
               
               do ip=1,l_max-l_min+1
                  pulse_str(igate,ip,i) = pulse(ip)
                  pulse_maxamp(igate,i)=max(abs(pulse_maxamp(igate,i)),
     .                 abs(pulse(ip)))
                  
                  if (pulse_maxamp(igate,i).eq.abs(pulse(ip))) then
                     pulse_maxind(igate,i)=ip+l_min-1
                     endif 
               enddo
               pulse_power(igate,i)=Sqrt(Sum(abs(pulse(:)*pulse(:))))


                
               filename = ''
               write(filename,'(A,i0,A)')
     .         'generated_pulses/Fourier_Pulse_gate',igate,'_i0.txt'


               open(igate+10,file=trim(filename))
               do ifreq=1,l_max-l_min+1
                  x = dble(ifreq+l_min-1)!/tau
                  write(igate+10,*) x,pulse_str(igate,ifreq,i)
!                  write(6,*) x,pulse_str(igate,ifreq,i)
...
!     This is the time domain pulse file writting

           
               call realtime_pulse(pulse_str(igate,:,i),l_min,l_max,tau
     .              ,0.d0,tau,ngate_on,n_disc,imax,pulset,igate,i)
!                write(6,*) i,'This is i'
               filename = ''
               write(filename,'(A,f0.16,A,i0,A)')'time_pulse/pulset_tau'
     .          ,tau,'gate',igate,'.txt'
               
               open(222+igate,FILE=trim(filename))
               do l=1,n_disc
                  write(222+igate,*) pulset(igate,l,i)
               enddo
               close(222+igate)
!               write(6,*) max(2.3d0,abs(-3.4d0))
!               stop
               val_max=0.d0
               do l=1, n_disc
                  val_max=max(val_max,abs(pulset(igate,l,i)))
!                  write(6,*) val_max
               enddo
               
               write(300+igate,*) tau, val_max
        

            DeAllocate(vec_null)
            DeAllocate(vec_null_ordered)
            DeAllocate(pulse)
            DeAllocate(vec_null_conc)
            DeAllocate(l_basis_freq)
            DeAllocate(ll_ordered)
       enddo
        r=r+1
  
      enddo
!      write(6,*) l_max,l_min
!     print *, abs(pulse1_str(:,10))
      ...
      do igate=1, num_gate
      do iimax=0,imax
         l_min = min(l_min,l_min_str(igate,iimax))
         
         l_max = max(l_max,l_max_str(igate,iimax))
...
!     l_range = max(l_max1-l_min1+1,l_max2-l_min2+1)

!      l_range1 = l_max1-l_min1+1
!      l_range2 = l_max2-l_min2+1


      Allocate(dmat_str(l_maxx-l_minn+1,l_maxx-l_minn+1))

 !     do ngst=0,2**num_gate-1
 !        write(filename_ent,'(A,i0,A,i0,A)')
 !    .      'entanglement/pattern',ngst,'.txt'
 !        open(ngst+10,file=trim(filename_ent))
 !     enddo
      ...
      do ion_a = 1,5
         do ion_b = ion_a+1,5
            m=m+1
            write(filename_ent,'(A,i0,A,i0,A)')
     .      'entanglement/ent_Q',ion_a,'_Q',ion_b,'.txt'
            open(m+50,file=trim(filename_ent))
        enddo
      enddo
!      do i=1, num_gate
!         print '((I4,A,I4))', index_gate(1,i),',',index_gate(2,i)
...
!     enddo
      ...
      m=0
      do iona = 1, num_qubit-1
         do ionb = iona+1, num_qubit
            m=m+1
            call dmat_creation(tau,freq,param_ld,iona,ionb,
     .           dmat_str,l_minn,l_maxx,l_minn,l_maxx)
            do i = 0, imax

               do ngst= nonbin_gate_stat, nonbin_gate_stat ! In order to sweep over all the gate configuratioins we need to start ngst from zero
!                  write(6,*) ibits(ngst,0,1),ibits(ngst,8,1)
  
                  do j=1,num_gate
                     gate_stat(j)=ibits(ngst,j-1,1)
                  enddo
c               do ngst1=0,1
c                  gate_stat(1)=ngst1
c                  do ngst2=0,1
...
c                                          gate_stat(9)=ngst9
c                                          do ngst10=0,1
c                                             gate_stat(10)=ngst10
               ent_ab=0.d0
               do igatea = 1, num_gate
                  do igateb = 1, num_gate
                     if (gate_stat(igatea).eq.1
     .                .AND.gate_stat(igateb).eq.1) then
                     
                  if(index_gate(1,igatea).eq.iona.OR.
     .                    index_gate(2,igatea).eq.iona) then
                     if(index_gate(1,igateb).eq.ionb.OR.
     .                    index_gate(2,igateb).eq.ionb) then
                        l_min_a = l_min_str(igatea,i)
                        l_max_a = l_max_str(igatea,i)
                        l_min_b = l_min_str(igateb,i)
                        l_max_b = l_max_str(igateb,i)
                        Allocate(pulse_a(l_max_a-l_min_a+1))
                        Allocate(pulse_b(l_max_b-l_min_b+1))
                        pulse_a=0.d0
                        pulse_b=0.d0
                        do ip = 1, l_max_a-l_min_a+1
                           pulse_a(ip) =  pulse_str(igatea,ip,i)
                        enddo
                        do ip = 1, l_max_b-l_min_b+1
                           pulse_b(ip) =  pulse_str(igateb,ip,i)
                        enddo
                        Allocate(dmat(l_max_a-l_min_a+1,
     .                  l_max_b-l_min_b+1))
                        call dmat_selector(dmat_str,l_min_a,l_max_a,
     .                       l_min_b,l_max_b,dmat,
     .                       l_minn,l_maxx,l_minn,l_maxx)
                        
                        x = DOT_PRODUCT(pulse_a,Matmul(dmat,pulse_b))
                        ent_ab = ent_ab + x
                        DeAllocate(dmat)
                        DeAllocate(pulse_a)
                        DeAllocate(pulse_b)
               ...
               do igatea = 1, num_gate
                  ...
                  if (gate_stat(igatea).eq.1) then
                     
                  if(index_gate(1,igatea).eq.iona.AND.
     .                    index_gate(2,igatea).eq.ionb) then
                        l_min_a = l_min_str(igatea,i)
                        l_max_a = l_max_str(igatea,i)
                        Allocate(pulse_a(l_max_a-l_min_a+1))
                        pulse_a=0.d0
     ...
     .                  l_max_a-l_min_a+1))
                        call dmat_selector(dmat_str,l_min_a,l_max_a,
     .                       l_min_a,l_max_a,dmat,
     .                       l_minn,l_maxx,l_minn,l_maxx)
                        
                        y = DOT_PRODUCT(pulse_a,Matmul(dmat,pulse_a))
                        ent_ab_ideal = ent_ab_ideal + y
...
!               ent_ab_ideal=0.d0
!               if(gate_stat(m).eq.1) ent_ab_ideal = datan(1.d0)/2.d0 ! sign missing
!               dev_ab = ent_ab - ent_ab_ideal
!     write(ngst+10,*) m,dev_ab
               total_power=SUM(gate_stat(:)*pulse_power(:,0))

               write(m+50,'(I4,A,f0.16,A,f0.16,A,f0.16,A)',advance='no')
     .         i, ', ', ent_ab,', ',ent_ab_ideal,', ',total_power,', '

!               print *, ent_ab_ideal
               
               do j = 1, num_gate
                  write(m+50, '(I1, A)', advance='no')
     .            gate_stat(num_gate+1-j)
               enddo
...
c               enddo
      ...
      ! This piece of code is to save the pulse power corresponding to each gate
      m=0
     
      write(filename_ent,'(A)')
     .'entanglement/pulse_power.txt'
      open(2,file=trim(filename_ent))
      do igate=1, num_gate
         write(2, '((f0.16,I4))') pulse_power(igate,0), igate
      enddo 
...
!     deallocating everything before the next iteration of tau

      DeAllocate(pulse_maxamp)
      DeAllocate(pulse_maxind)
      DeAllocate(pulse_power)

c      Allocate(ndim_pulse1_str(0:imax))
      DeAllocate(l_min_str)
      DeAllocate(l_max_str)
!      Allocate(pulse2_str(l_pulse_dim,0:imax))
     

      DeAllocate(pulse_str)
      ...
      function nchoose(n,l) result(m)
      integer, intent(in) :: n,l
      integer :: m
      m=nfact(n)/(nfact(l)*nfact(n-l))
      end function nchoose

     
      
      recursive function nfact(n) result(m)
      ! this function is to calculate the factorial of an integer
        integer, intent(in) :: n
        integer :: m
      ...
      subroutine dmat_selector(dmat_str,l_min1,l_max1,l_min2,l_max2,
     .     dmat,l_minn1,l_maxx1,l_minn2,l_maxx2)
      implicit double precision (a-h,o-z)
      parameter (num_ion=7,num_qubit=5,num_gate=2)
      dimension dmat_str(l_maxx1-l_minn1+1,l_maxx2-l_minn2+1)
      dimension dmat(l_max1-l_min1+1,l_max2-l_min2+1)

      do i2=1,l_max2-l_min2+1
      do i1=1,l_max1-l_min1+1
         dmat(i1,i2) = dmat_str(i1+l_min1-l_minn1,i2+l_min2-l_minn2)
      enddo
      enddo
      ...
      subroutine realtime_pulse(pulsef,l_min,l_max,tau,t1,t2,ngate_on,n,
     .imax,pulset,igate,i)
      implicit double precision (a-h,o-z)
      parameter (num_ion=7,num_qubit=5,num_gate=10)
      dimension pulsef(l_max-l_min+1)
      dimension pulset(num_gate,n,0:imax)
      double precision, allocatable :: sin_array(:)

      allocate(sin_array(l_max-l_min+1))
...
!      stop
      ...
      do ntstp=1,n
         t=(ntstp-1)*(t2-t1)/(n-1)+t1
         do l=1,l_max-l_min+1
            lp=l+l_min-1
            frq=lp*2.d0*pi/tau
            sin_array(l)=dsin(frq*t)
         enddo

         pulset(igate,ntstp,i)=sum(pulsef(:)*sin_array(:))
      enddo 
!      open(222,FILE='fin')
!      do l=1,l_max-l_min+1
!         write(222,*)  sin_array(l)
!      enddo
!      close(222)
      ...
      subroutine dmat_creation(tau,freq,param_ld,ion1,ion2,
     .dmat_str,l_minn1,l_maxx1,l_minn2,l_maxx2)
...
c     =============================================================
      ...
      dimension freq_range(2,num_gate)
!      dimension dmat(l_max1-l_min1+1,l_max2-l_min2+1)
!     dimension smat(l_max1-l_min1+1,l_max2-l_min2+1)
      dimension dmat_str(l_maxx1-l_minn1+1,l_maxx2-l_minn2+1)
...
!      num_basis2 = l_max2-l_min2+1
      ...
      num_basis1=l_maxx1-l_minn1+1
      num_basis2=l_maxx2-l_minn2+1
      ...
      do kp=1,num_basis1
         ...
         do lp=1,num_basis2
            ...
            do ip=1,num_ion
               if(k.eq.l) then
       dint = tau**2*(-4.d0*l**2*pi**2*tau*freq(ip,1)
     . +tau**3*freq(ip,1)**3-8.d0*l**2*pi**2*dsin(tau*freq(ip,1)))
     . /(2.d0*(-4.d0*l**2*pi**2+tau**2*freq(ip,1)**2)**2)

!(\[Tau]^2 (-4 l^2 \[Pi]^2 \[Tau] \[Omega] + \[Tau]^3 \[Omega]^3 - 
!   8 l^2 \[Pi]^2 Sin[\[Tau] \[Omega]]))/(2 (-4 l^2 \[Pi]^2 + \[Tau]^2 \
!\[Omega]^2)^2)               This is what we doubt is different from the expression below!
...
!\[Omega]^2)^2)
      ...
      dint = -(4.d0*k*l*pi**2*dsin(tau*freq(ip,1)))/
     .        ((4.d0*k**2*pi**2-tau**2*freq(ip,1)**2)*
     .        (4.d0*l**2*pi**2-tau**2*freq(ip,1)**2))      


!-((4 k l \[Pi]^2 \[Tau]^2 Sin[\[Tau] \[Omega]])/((4 k^2 \[Pi]^2 - \
!\[Tau]^2 \[Omega]^2) (4 l^2 \[Pi]^2 - \[Tau]^2 \[Omega]^2)))       This is what we doubt is different from the expression below!
...
!\[Omega]^2) (4 l^2 \[Pi]^2 - \[Tau]^2 \[Omega]^2)))
               ...
               dmat_str(lp,kp) = dmat_str(lp,kp) + 
     .         param_ld(ip,ion1,1)*param_ld(ip,ion2,1)*dint
            enddo
         enddo
      enddo

!      dmat = 0.5d0*(dmat + Transpose(dmat))
...
!     dimension vec_null(l_max-l_min+1,l_max-l_min+1-nrow_len)
      dimension vec_null(l_max-l_min+1,l_fi-l_in+1)
      dimension pulse(l_max-l_min+1)
      dimension smat(l_max-l_min+1,l_max-l_min+1)
      dimension dmat(l_max-l_min+1,l_max-l_min+1)
      dimension A(l_fi-l_in+1,l_fi-l_in+1)
      dimension U(l_fi-l_in+1,l_fi-l_in+1)
      dimension VT(l_fi-l_in+1,l_fi-l_in+1)
      dimension S(l_fi-l_in+1)
...
! hmat_ij = \sum_p \eta_p,ion1 \eta_p,ion2 int_0^tau dt \int_0^t dt' sin(omega_p(t'-t)) sin(2*i*pi*t/tau) sin(2*j*pi*t/tau)
! The double integral = 

      smat = 0.5d0*(dmat + Transpose(dmat))

      ! rmat := null-space reduced smat
      A = Matmul(Matmul(Transpose(vec_null),smat),vec_null)

      ! svd of rmat to find the largest EV
c      nm = num_basis-num_ion
c      call svd(nm,nm,nm,
...
*
     ...
     $             WORK, LWORK, INFO )
      ...
      ! Probably unnecessary? S is signless afaik.
      if(dabs(S(1)).gt.dabs(S(N))) then
         isolution = 1
      else
         isolution = N
      endif
...
c      stop
      ! Check the angle
c      write(6,*) DOT_PRODUCT(pulse,Matmul(smat,pulse)),S(isolution)
...
c
      subroutine null_space
     .(tau,freq,vec_null,l_min,l_max,nrow_len,num_null_dim)
c     ====================================================
      implicit double precision (a-h,o-z)
      parameter (num_ion=7,num_qubit=5,num_gate=10)
!      integer, parameter ::  nrow_len = num_ion * (norder + 1)
!      double precision seed(nrow_len,l_max-l_min+1) 
      parameter (LWMAX = 1000000)
      dimension freq(num_ion,3)
      dimension A(nrow_len,l_max-l_min+1)
      dimension Acopy(nrow_len,l_max-l_min+1)
...
c----------------------------------------
      nstab_order=int(nrow_len/num_ion)-1
      num_basis = l_max-l_min+1
      ! phase-space closure matrix (A)
      ! h_p^{(-)}(t) needs to be killed
      A = 0.d0
      pi = 4.d0*datan(1.d0)
      do 1 icol=1,l_max-l_min+1
         pil = pi*(icol+l_min-1)
      do 2 irow=1,num_ion
         omega = freq(irow,1)
         arg = tau*omega/2.d0
      if(nstab_order.eq.0) then
          A(irow,icol) = tau*(tau*omega*dcos(arg)*dsin(2.d0*pil)-
     .           4.d0*pil*(dcos(pil))**2*dsin(arg))
     .        /(-4.d0*pil**2+(tau*omega)**2)
! Int_0^tau sin(2*pi*l*t/tau) sin[\omega_p (\tau/2-t)] dt = 
! (\[Tau] (\[Tau] \[Omega] Cos[(\[Tau] \[Omega])/2] Sin[2 l \[Pi]] - 
!   4 l \[Pi] Cos[l \[Pi]]^2 Sin[(\[Tau] \[Omega])/
!     2]))/(-4 l^2 \[Pi]^2 + \[Tau]^2 \[Omega]^2)
       elseif(nstab_order.eq.1) then
...
!      Write(6,*) seed(2,:), 'This is my random number '
...
!       write(6,*) DOT_PRODUCT(A(6,:),A(7,:)),'This is the dot product '
...
!     stop
      ...
      call DGESVD ( 'All', 'All', M, N, Acopy, LDA, S, U, LDU,
     $                   VT, LDVT, WORK, LWORK, INFO )
      LWORK = MIN( LWMAX, INT( WORK( 1 ) ) )
*
*     Compute SVD.
*
      CALL DGESVD( 'All', 'All', M, N, Acopy, LDA, S, U, LDU, VT, LDVT,
     $             WORK, LWORK, INFO )
...
!         vec_test2=vec_test2 - matmul(transpose(A),
!     .   matmul(A,vec_null(:,inull)))
!         write(6,*) inull,norm2(vec_test)
c         if(norm2(vec_test).gt.1.d-9) then
c            write(6,*) 'Check the null space vectors'
c!           print *, norm2(vec_test)
c            stop
c     endif
         if(norm2(vec_test).lt.pthres) then
            cycle
         else
            exit
         endif
...
c         endif
      ...
      enddo
      num_null_dim = num_basis-inull
c      write(6,*) num_null_dim
c     stop
      write(6,*) num_null_dim,'num null dim'
      if(num_null_dim.lt.1) then
         write(6,*) 'no null space'
         stop
      endif
!      stop
