...
! omega_p; angular freq as default
         pi = 4.d0*datan(1.d0)
         mode_freq_range =  freq(num_ion,1)- freq(1,1)
         freq_range_1 = freq(1,1)-1.5d0*mode_freq_range 
         freq_range_2 = freq(num_ion,1)+1.5d0*mode_freq_range 
...
!            nrow_len=num_ion*(nstab_order+1)
     ...
     .       (tau,freq,vec_null2,l_min,l_max,nrow_len,num_null_dim)
            ...
            l_in=1+nint(dble(r)*dble(num_null_dim)/dble(ngate_on))
            l_fi=nint(dble(r+1)*dble(num_null_dim)/dble(ngate_on))
            write(6,*) l_in,l_fi
c            stop
            ...
            do ivec=l_in,l_fi
               vec_null2(:,ivec-l_in+1) = vec_null(:,ivec)
            enddo
