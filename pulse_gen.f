...
c-----------------------------------------------------------
      ...
      freq_range(1,1) = 2.400d0; freq_range(2,1) = 2.960d0
      freq_range(1,2) = 3.066d0; freq_range(2,2) = 3.550d0

      ! test with the full range
!      freq_range(1,1) = 2.400d0; freq_range(2,1) = 3.501d0

      do igate=1,num_gate
         if(freq_range(2,igate).lt.freq_range(1,igate)) then   !if expression is not true, excute the following lines
      ...
      ! qubit 1, mode x
      param_ld(1,1,1) = -0.0084d0
      param_ld(2,1,1) = -0.0248d0
      param_ld(3,1,1) = -0.0372d0
      param_ld(4,1,1) = +0.0445d0
      param_ld(5,1,1) = -0.0334d0
      param_ld(6,1,1) = +0.0059d0
      param_ld(7,1,1) = +0.0565d0
      ! qubit 2, mode x
      param_ld(1,2,1) = +0.0279d0
      param_ld(2,2,1) = +0.0448d0
      param_ld(3,2,1) = +0.0303d0
      param_ld(4,2,1) = +0.0053d0
      param_ld(5,2,1) = -0.0325d0
      param_ld(6,2,1) = +0.0333d0
      param_ld(7,2,1) = +0.0385d0
      ! qubit 3, mode x
      param_ld(1,3,1) = -0.0522d0
      param_ld(2,3,1) = -0.0203d0
      param_ld(3,3,1) = +0.0301d0
      param_ld(4,3,1) = -0.0313d0
      param_ld(5,3,1) = -0.0039d0
      param_ld(6,3,1) = +0.0398d0
      param_ld(7,3,1) = +0.0285d0
      ! qubit 4, mode x
      param_ld(1,4,1) = +0.0496d0
      param_ld(2,4,1) = -0.0352d0
      param_ld(3,4,1) = -0.0166d0
      param_ld(4,4,1) = -0.0341d0
      param_ld(5,4,1) = +0.0176d0
      param_ld(6,4,1) = +0.0391d0
      param_ld(7,4,1) = +0.0161d0
      ! qubit 5, mode x
      param_ld(1,5,1) = -0.0213d0
      param_ld(2,5,1) = +0.0426d0
      param_ld(3,5,1) = -0.0446d0
      param_ld(4,5,1) = +0.0121d0
      param_ld(5,5,1) = +0.0381d0
      param_ld(6,5,1) = +0.0298d0
      param_ld(7,5,1) = +0.0122d0
      ! freqeuencies, mode x
      freq(1,1) = 2.95591d0
      freq(2,1) = 2.97371d0
      freq(3,1) = 2.99241d0
     ...
     .   'Fourier_Pulse_Q',ion1,'_Q',ion2,'.txt'
